...
if (isset($_POST['update'])) {
    ...
    $up1 = mysqli_query($conn, "UPDATE anggota SET no_telp = '$no_telp_baru' WHERE id_anggota = '$id_agt'");

    // Update Password jika diisi
    $up2 = true; // default true
    if (!empty($pass_baru)) {
        $pass_hash = md5($pass_baru);
        $up2 = mysqli_query($conn, "UPDATE users SET password = '$pass_hash' WHERE username = '$username_session'");
    }

    if ($up1 && $up2) {
        // GANTI BAGIAN ECHO ALERT LAMA DENGAN INI:
        tampilkanAlert('Profil Diperbarui!', 'Data akun Anda telah berhasil disimpan.', 'success', 'profil.php');
    } else {
        tampilkanAlert('Gagal!', 'Terjadi kesalahan saat menyimpan data.', 'error', 'profil.php');
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Profil Saya - Librarify</title>
